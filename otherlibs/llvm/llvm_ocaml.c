...
CAMLprim value llvm_param_types(LLVMTypeRef FunTy) {
  value Tys = caml_alloc(LLVMCountParamTypes(FunTy), 0);
...
CAMLprim value llvm_struct_element_types(LLVMTypeRef StructTy) {
  value Tys = caml_alloc(LLVMCountStructElementTypes(StructTy), 0);
...
CAMLprim value llvm_get_namedmd(LLVMModuleRef M, value Name)
{
  CAMLparam1(Name);
  CAMLlocal1(Nodes);
  Nodes = caml_alloc(LLVMGetNamedMetadataNumOperands(M, String_val(Name)), 0);
...
CAMLprim value llvm_params(LLVMValueRef Fn) {
  value Params = caml_alloc(LLVMCountParams(Fn), 0);
...
CAMLprim value llvm_basic_blocks(LLVMValueRef Fn) {
  value MLArray = caml_alloc(LLVMCountBasicBlocks(Fn), 0);
